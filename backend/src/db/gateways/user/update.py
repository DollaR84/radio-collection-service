...
class UpdateUserGateway(BaseGateway[int, User]):
    ...
    async def update_user_by_id(self, user_id: int, update_data: domain.UpdateUserModel) -> int:
        error_message = f"Error update user by id='{user_id}'"
        stmt = update(User)
    ...
    async def update_user_by_uuid(self, uuid_id: uuid.UUID, update_data: domain.UpdateUserModel) -> int:
        error_message = f"Error update user by uuid='{uuid_id}'"
        stmt = update(User)
    ...
    async def update_user_by_google(self, google_id: str, update_data: domain.UpdateUserModel) -> int:
        error_message = f"Error update user by google_id='{google_id}'"
        stmt = update(User)
    ...
    async def update_user_by_email(self, email: str, update_data: domain.UpdateUserModel) -> int:
        error_message = f"Error update user by email='{email}'"
        stmt = update(User)
...
class UpdateAccessPermissionGateway(BaseGateway[int, AccessPermission]):

    async def update_permission(self, permission_id: int, update_data: domain.UpdateAccessPermissionModel) -> int:
        error_message = f"Error update permission for user by id='{update_data.user_id}'"
        stmt = update(AccessPermission)
        stmt = stmt.where(AccessPermission.id == permission_id)

        stmt = stmt.values(**update_data.dict(exclude_unset=True))
        stmt = stmt.returning(AccessPermission.id)
